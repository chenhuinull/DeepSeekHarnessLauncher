...
static int RunChecks(int argc, wchar_t** argv) {
    ...
    // these checks read the region, and the window rectangle is checked to stay exactly as it was.
    {
        ...
        if (!probe) {
            ...
            // remote desktop reading the window's own surface showed the whole window around the chip (a bar
            // the width of the window, black before the empty part of the bitmap was filled white and white
            // after). The region is the shape; the alpha inside it still draws the corners, and the clip is a
