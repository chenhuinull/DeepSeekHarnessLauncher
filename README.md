...
Windows 桌面启动器，提供启动/停止、更新、窗口始终置顶及可折叠运行日志。标题栏从左到右依次是关闭、最小化、置顶、更新、“日志”、启动/停止、状态灯，最后是鲸鱼图标——窗口按钮与鲸鱼图标本来各在两端（鲸鱼在左、最小化/关闭在右），按用户要求调换了位置，且关闭与最小化再互换一次（现在是关闭打头）、状态灯也从最左挪到了鲸鱼旁边、中间四个按钮再排成“置顶、更新、日志、启动/停止”（这样“启动/停止”紧挨着报告结果的状态灯）：“状态灯”悬停可查看当前状态说明，“日志”控制下方日志文本框，日志默认折叠；“置顶”开启时按钮为蓝色底，“更新”有新版本时为紫色底、可回退时为橙色底。双击鲸鱼图标可以把窗口折叠成只剩状态灯和鲸鱼图标的窄条（按住鲸鱼图标或标题栏空白处都能拖动窗口），再双击图标即可展开并恢复折叠前的日志状态；折叠后左边是状态灯、右边是鲸鱼图标（与展开时“灯在鲸鱼左边”的顺序一致），窄条因此是 82px 而不是 84px。**折叠方向是从左到右**：折叠时左边缘往右收、右边缘原地不动（`Layout()` 用 `current.right - width` 定位），所以灯和鲸鱼在屏幕上一动不动——它们到右边缘的距离在两种形状下相同，正好在鼠标指针下面；如果改成钉住左边缘，鲸鱼会从标题栏右端跳到左端。反向展开也是这样长回左边原处。**折叠不再闪烁**：折叠会把整个窗口平移 262px，而重绘原来是排队等消息循环处理的——合成器会先把“旧画面放在新位置”显示至少一帧，看起来就是闪一下（这正是改成从左到右折叠之后才出现的）。现在 `Layout()` 用 `SetWindowRgn(..., FALSE)` 免掉系统那次多余的重绘，收尾用 `InvalidateRect` + `UpdateWindow` **同步**画完再返回，移动与新画面落在同一帧里；折叠时也不再走两次 `Layout()`（`ToggleMini` 直接置 `expanded = false`）。回归检查里有对照：排队式重绘在调用内绘制 0 次（旧行为），`Layout()` 则为 1 次。**圆角统一为 3px**：按钮、窗口按钮底板、右键菜单高亮、日志框和窗口外框共用同一个 `cornerRadius`，窗口区域用的椭圆尺寸取它的两倍（`CreateRoundRectEllipse` 语义），这样窗口形状与描边曲率一致——此前外框是 6px、控件是 4px，看起来像两种形状。窗口可拖动、最小化和关闭；启动器是工具窗口，不会出现在任务栏和 Alt+Tab 列表中。启动后窗口默认显示，并自动启动服务（若已有服务在运行则直接接管，不会重复启动）。托盘图标始终存在：左键单击显示/隐藏窗口，右键单击弹出菜单（显示/隐藏窗口、启动/停止服务、自动重启（勾选项，开启后运行中的状态灯变紫）、退出启动器）；最小化按钮把窗口收回托盘，关闭按钮退出启动器（Web 服务继续运行）。再次运行 exe 也能把它从托盘唤回（单实例保护会显示并激活已有窗口）。
