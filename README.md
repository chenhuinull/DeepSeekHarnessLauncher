...
Windows 桌面启动器，提供启动/停止、更新、窗口始终置顶及可折叠运行日志。标题栏从左到右依次是关闭、最小化、启动/停止、“日志”、“更新”、“置顶”、状态灯，最后是鲸鱼图标——窗口按钮与鲸鱼图标本来各在两端（鲸鱼在左、最小化/关闭在右），按用户要求调换了位置，且关闭与最小化再互换一次（现在是关闭打头）、状态灯也从最左挪到了鲸鱼旁边：“状态灯”悬停可查看当前状态说明，“日志”控制下方日志文本框，日志默认折叠；“置顶”开启时按钮为蓝色底，“更新”有新版本时为紫色底、可回退时为橙色底。双击鲸鱼图标可以把窗口折叠成只剩状态灯和鲸鱼图标的窄条（按住鲸鱼图标或标题栏空白处都能拖动窗口），再双击图标即可展开并恢复折叠前的日志状态；折叠后左边是状态灯、右边是鲸鱼图标（与展开时“灯在鲸鱼左边”的顺序一致），窄条因此是 82px 而不是 84px。窗口可拖动、最小化和关闭；启动器是工具窗口，不会出现在任务栏和 Alt+Tab 列表中。启动后窗口默认显示，并自动启动服务（若已有服务在运行则直接接管，不会重复启动）。托盘图标始终存在：左键单击显示/隐藏窗口，右键单击弹出菜单（显示/隐藏窗口、启动/停止服务、自动重启（勾选项，开启后运行中的状态灯变紫）、退出启动器）；最小化按钮把窗口收回托盘，关闭按钮退出启动器（Web 服务继续运行）。再次运行 exe 也能把它从托盘唤回（单实例保护会显示并激活已有窗口）。
