...
Windows 桌面启动器，提供启动/停止、检查更新、窗口始终置顶及可折叠运行日志。标题栏从左到右依次是状态灯、启动/停止、“日志”、检查更新、置顶开关和窗口按钮：“状态灯”悬停可查看当前状态说明，“日志”控制下方日志文本框，日志默认折叠。双击鲸鱼图标可以把窗口折叠成只剩图标和状态灯的窄条（按住鲸鱼图标或标题栏空白处都能拖动窗口），再双击图标即可展开并恢复折叠前的日志状态。窗口可拖动、最小化和关闭。
